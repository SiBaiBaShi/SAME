...
def update_index(url, sheet):
    ...
    sheet.range('A1').api.EntireRow.Delete()
    for x in range(1, len(excel_list)):  # 第一个必然是今日的数据，但今日的不写入excel
        sheet.range('A'+str(x)).value = excel_list[x]
...
def download_from_index(the_url, sheets, time_list, value_dict):
    ...
    for x in range(len(total_list)):
        if total_list[x][0] is None:
            ...
            return 0
        # 索引excel中保存的是每一天零点时的秒时间，而time_to是该天24时的时间，因此需要减一天的时间才能匹配上
        elif total_list[x][1] >= time_list[1] / 100-864 > total_list[x+1][1]:
